...
import { fillModes, fonts } from '../constants';
    ...
    <div className="control-section">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 15 }}>
        ...
        <div className="input-group" style={{ flex: 1, minWidth: 180 }}>
          ...
          <select id="fill-mode" value={fillMode} onChange={(e) => setFillMode(e.target.value)}>
            {fillModes.map((mode) => (
    ...
    <div className="control-panel text-settings">
      ...
      <div className="control-section">
        ...
        {textItems.map((item) => (
          <div
            key={item.id}
            className="text-item-settings control-section"
            style={{
              padding: '12px',
          ...
          >
            ...
            >
              <h3 style={{ margin: 0, fontSize: '1rem', color: 'var(--primary-dark)' }}>
                テキスト #{textItems.indexOf(item) + 1}
              </h3>
              {textItems.length > 1 && (
                <button
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
              <div className="input-group" style={{ flex: 1, minWidth: 120 }}>
                <label htmlFor={`text-color-${item.id}`} style={{ fontSize: 13, marginBottom: 3 }}>
                  テキスト色
                </label>
            ...
            <div className="input-group">
              <label htmlFor={`rotate-${item.id}`} style={{ fontSize: 13, marginBottom: 3 }}>
                回転角度: {item.rotate || 0}°
              </label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button
                  onClick={() =>
                    updateTextItem(item.id, { rotate: Math.max(-45, (item.rotate || 0) - 5) })
                  }
                  style={{
                    padding: '2px 6px',
                ...
                <button
                  onClick={() =>
                    updateTextItem(item.id, { rotate: Math.min(45, (item.rotate || 0) + 5) })
                  }
                  style={{
                    padding: '2px 6px',
                    backgroundColor: '#f0f0f0',
